...
os.environ["TF_CPP_MIN_LOG_LEVEL"] = "3" # ignore TF log  os.environ["CUDA_VISIBLE_DEVICES"] = "3"
...
print("Loading single reasoning module...")
# deduction & abduction modules
...
reasoning_modules = {'substitution':{'model': module_all.to(device), 'task_prefix': 'deductive substitution:'},
                     'conjunction': {'model':module_all.to(device), 'task_prefix':'deductive conjunction:'},
                     'if-then':{'model':module_all.to(device), 'task_prefix':'deductive if-then:'}}
reasoning_modules_abd = {'substitution_abd':{'model':module_all.to(device), 'task_prefix':'abductive substitution:'},
                         'conjunction_abd': {'model':module_all.to(device), 'task_prefix':'abductive conjunction:'},
                         'if-then_abd':{'model':module_all.to(device), 'task_prefix':'abductive if-then:'}}
        ...
        for step_pre, step_con in state_future_steps:
            if all([idx in pre for idx in step_pre]):
                ...
                if len(step_pre) == 2:
                    ...
                    sent1 = get_node(step_pre[0], state_tree)['sent']
                    sent2 = get_node(step_pre[1], state_tree)['sent']
                    # print(sent1)
                    # print(sent2)
                    pred_label = rhe_model([sent1], [sent2], rhe_rep, rhe_tokenizer)
                    gold_step_rhe.append(pred_label.item())
                    # if pred_label[0] != 1:
                    #     input(pred_label)
                if len(step_pre) > 2:
...
name_list = [f'../data/Controller_data/train.controller.{task_name}.{version_name}.jsonl',
...
for process_data, save_name in zip(data_list, name_list):
    ...
    for data_item in tqdm(process_data):
        ...
        data_item['controller_data'] = {'hypothesis': data_item['hypothesis'],
        ...
        for state in candidate_states[:max_state_pre_data_item]:
            ...
            for pre_idx in state['pre']:
                ...
                if buffer_key not in infered_buffer:
                    ...
                    output_sents, output_types = inference_alltype_batch(input_sents, (reasoning_modules,
                                                                                       tokenizer_module, args_module))
                ...
                inter_scores = bleurt_scorer.score(references=[gold_inter_sents]*len(output_sents),
                                                   candidates=output_sents)
                ...
                for sent_, score_ in zip(output_sents, inter_scores):
                    if score_ == max(inter_scores) and score_ > pos_bleurt_thre:
                        ...
                        new_gold_next_steps = []
                        new_gold_next_steps_rhe = []
                        for step_idx, step_ in enumerate(now_state['gold_next_steps']):
                            new_gold_next_steps_rhe.append(now_state['gold_steps_rhe'][step_idx])
                        ...
                        states_positive_true_step.append({
                                ...
                                'gold_steps_rhe':new_gold_next_steps_rhe,
                    ...
                    if score_ < neg_bleurt_thre:
                        ...
                        states_negative_true_step.append({
                                ...
                                'gold_steps_rhe':now_state['gold_steps_rhe'],
        ...
        for state in candidate_states[:max_state_pre_data_item]:
            ...
            for sent_, score_ in zip(output_sents, inter_scores):
                if score_ == max(inter_scores) and score_ > pos_bleurt_thre:
                    ...
                    new_gold_next_steps_rhe = now_state['gold_steps_rhe']
