        ...
        for index, input_ in enumerate(tmp_sents):
            ...
            if buffer_key in buffer_dict[module_type].keys():
                ...
                with torch.no_grad():
                    ...
                    llm_output = llm_model.generate(instruct_ids, max_new_tokens=500, num_return_sequences=num_return, do_sample=True)
                    llm_output = llm_tokenizer.batch_decode(llm_output)
                    llm_output = [item.replace("<pad> ", "").replace("<pad>", "").replace("</s>", "") for item in llm_output]
