...
class controller_compare_sample_dataset(Dataset):
        ...
        for data_item in datas:
            ...
            for state1 in compare_pool_pos:
                ...
                for state2 in states_compare_with_state1:
                    ...
                    if info_item1 is None or info_item2 is None: continue
    ...
    def make_compare_within_item(self, info_item, state):
        ...
        compare_step_within_idx1_rhe = []
        compare_step_within_idx1_rhe_mask = []
        begin_idx = 0
        compare_step_within_idx2 = []
        compare_step_within_label = []
        compare_step_within_info = []

        for step_idx, pos_step in enumerate(info_item['positive_next_steps']):
            pos_step_rhe = info_item['positive_next_steps_rhe'][step_idx]
            for neg_step in info_item['negative_next_steps']:
                idx1 = all_steps.index(sorted(pos_step))
                idx2 = all_steps.index(sorted(neg_step))
                # print(pos_step)
                # input(neg_step)
                compare_step_within_idx1_rhe.append(pos_step_rhe)
                compare_step_within_idx1_rhe_mask.append(0)
            ...
            if len(compare_step_within_idx1_rhe_mask) > 0:
                compare_step_within_idx1_rhe_mask[begin_idx] = 1
                begin_idx = len(compare_step_within_idx1_rhe_mask)
    ...
    def make_item(self, data_item, state):
        ...
        if state['state_label'] == 1:
            ...
            positive_next_steps_rhe = state['gold_steps_rhe']
        ...
        info_item = {
            ...
            'positive_next_steps_rhe':positive_next_steps_rhe,
...
class LinearsHead(nn.Module):
    def __init__(self, config, input_num=2, output_num=1, use_act=False):
        super(LinearsHead, self).__init__()
        self.linear1 = nn.Linear(input_num * config.hidden_size, config.hidden_size)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.linear2 = nn.Linear(config.hidden_size, output_num)
...
class Controller(nn.Module):
    def __init__(self, args, config):
        ...
        self.rhe_stephead = LinearsHead(config, input_num=2, output_num=3, use_act=True)
    ...
    def score_instance(self, batch_encoder_inputs, batch_info):
        ...
        for batch_index in range(batch_size):   
            ...
            if len(fact_features) > 1:
                ...
                pred_step_rhe_score = self.rhe_stephead(step_features)
            ...
            batch_pred_step_rhe_score.append(pred_step_rhe_score)
        ...
        return {
            ...
            'batch_pred_step_rhe_score':batch_pred_step_rhe_score,
    ...
    def training_pair(self, batch_encoder_inputs, batch_info):
        ...
        batch_pred_step_rhe_score = score_results['batch_pred_step_rhe_score']
        ...
        loss_rhe_step = []
        for k in range(batch_size):
            compare_step_within_idx1, compare_step_within_idx2, compare_step_within_label, step_rhe, step_rhe_mask \
                = batch_info[k]['compare_step_within']
            pred_step_score_state = batch_pred_step_score[k]
            pred_step_rhe_score_state = batch_pred_step_rhe_score[k]
            ...
            # nll loss
            pred_step_rhe = pred_step_rhe_score_state[compare_step_within_idx1]
            target = torch.tensor(step_rhe).to(self.device)
            step_rhe_mask = torch.tensor(step_rhe_mask).to(self.device)
            loss_rhe = F.nll_loss(pred_step_rhe, target, reduction='none')
            masked_loss_rhe = torch.sum(loss_rhe * step_rhe_mask, -1) / torch.sum(step_rhe_mask, -1)
            loss_rhe_step.append(masked_loss_rhe)
        loss_step = sum(loss_step) / len(loss_step) if len(loss_step) else torch.tensor(0).to(self.device)
        loss_rhe_step = sum(loss_rhe_step) / len(loss_rhe_step) if len(loss_rhe_step) else torch.tensor(0).to(self.device)
...
def tokenize_target_fact_sents(target_sents, fact_sents, tokenizer, pad_max_length = None):
    ...
    # print("target", target_sents)
    # print("fact: ", fact_sents)
    # input()
...
def train_one_step(batch, model, tokenizer, args):
    ...
    batch_label_info = []
    for item in batch:
        input_ids, attention_mask, target_offsets, fact_offsets = tokenize_target_fact_sents(item['target_sents'],
                                                                                             item['fact_sents'],
                                                                                             tokenizer,
        ...
        for batch in data_loader:
            ...
            for item in batch:
                input_ids, attention_mask, target_offsets, fact_offsets = \
                    tokenize_target_fact_sents(item['target_sents'], item['fact_sents'], tokenizer,
                                               pad_max_length=args.max_token_length)
                batch_encoder_inputs['input_ids'].append(input_ids)
                batch_encoder_inputs['attention_mask'].append(attention_mask)
...
def load_controller(exp_dir, model_name=None, retrieve_learning=False, device=None):
    # read config
    ...
    # load model
    if retrieve_learning:
        from tune_Controller import RetrieveM
        model = RetrieveM(args, model_config).to(device)
        model.load_state_dict(state_dict)
        return model.controller_model_.state_dict()
    else:
        return state_dict
...
def run(args):
    ...
    # load_model
    # pre_trained_steps = 0  # We train based on theirs
    if args.load_pre_trained_model and args.exp_dir_controller is not None:
        model_dict = model.state_dict()

        state_dict = load_controller(args.exp_dir_controller, args.model_name_controller, args.retrieve_learning, device)

        model_dict.update(state_dict)
        model.load_state_dict(model_dict)
        log.info(f"Resume model parameters form {args.exp_dir_controller} model name {args.model_name_controller}")
    ...
    for epoch_i in range(epoch_init, args.epochs+1):
        ...
        for batch in train_loader:
            ...
            loss_rhe_step = model_return['loss_rhe_step']
            ...
            if epoch_i >= args.consider_rst:
                loss = loss + args.loss_weight[4] * loss_rhe_step
            loss.backward()
...
def get_params():
    ...
    parser.add_argument('--load_pre_trained_model', action='store_true', default=False)
    parser.add_argument('--report_epoch', type=float, default=1.0)
    parser.add_argument("--exp_dir_controller", type=str,  default=None)
    parser.add_argument("--model_name_controller", type=str,  default=None)  
    parser.add_argument('--consider_rst', type=int, default=100000000000)
    parser.add_argument('--retrieve_learning', action='store_true', default=False)
